#!/usr/bin/env python3
"""
Fit a 2-component full-covariance GMM on default library instant features for one leg,
project to PCA PC1–PC2 with per-axis σ scaling for a square view, and plot in a style
similar to a classic GMM+PCA figure: viridis log-density contours, red/blue scatter by
stance vs swing (from mixture mean ``p_foot_body_z``: higher z → swing).

**Contours (the colored curves):** Each line is where the **fitted 2-component Gaussian
mixture** assigns the same **log probability density** to points in the 2D PCA plane (after
σ-scaling). Brighter / yellow-ish curves are **higher** density (more “typical” kinematic
instants under the model); dark purple curves are **lower** density (rarer combinations of
calf torque, foot velocities, and foot height). They are **not** decision boundaries between
stance and swing; they show the **shape of the mixture** (two bumps) in PC space.

Example::

    cd /path/to/async_ekf_workspace
    MPLBACKEND=Agg python3 scripts/visualize_gmm_pca_split.py --save gmm_pca.png
"""
...
def _logpdf_grid_scaled_pc(
    ...
    """Mixture log-density on a grid in σ-scaled PC space (map back to R^5 for score_samples)."""
    zs0 = xx.ravel()
    zs1 = yy.ravel()
    z_un = np.column_stack([zs0 * s0, zs1 * s1])
    x5 = pca.inverse_transform(z_un)
    logp = gmm.score_samples(x5)
    return logp.reshape(xx.shape)


def main() -> None:
    default_seq = "/home/girgine/Documents/leg-odometry/iros/data_anymal"
    ap = argparse.ArgumentParser(description="2-GMM on default features + PCA scatter (one leg, full sequence).")
    ...
    iz = spec.fields.index("p_foot_body_z")
    m_sk = np.asarray(gmm.means_, dtype=np.float64)
    stance_sk = int(np.argmin(m_sk[:, iz]))
    if int(np.argmax(m_sk[:, iz])) == stance_sk:
        raise SystemExit("GMM components degenerate: identical mean foot height; cannot split stance/swing.")
    pred = gmm.predict(X)
    labels = np.where(pred == stance_sk, 0, 1)
    ...
    with plt.rc_context(rc=_REFERENCE_RC):
        ...
        #ax.set_title(f"GMM Fit for Leg {args.leg}", fontsize=13, pad=4)
        ...
        #secy.set_ylabel("PCA Component 2")
