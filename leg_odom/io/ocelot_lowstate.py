"""
Load Ocelot lowstate layout: one ``lowstate.csv`` per trajectory directory.

V1 contract:
- required: ``lowstate.csv``
...
def _required_lowstate_columns() -> tuple[str, ...]:
    """
    Required columns for Ocelot lowstate V1.

    ``motor_i_ddq`` is intentionally optional in V1.
    """
    return (
        TIME_SEC_COL,
        TIME_NANOSEC_COL,
...
def _assert_required_columns(df: pd.DataFrame) -> None:
    missing = [c for c in _required_lowstate_columns() if c not in df.columns]
    if missing:
        raise KeyError(
            "lowstate.csv missing required columns: "
            + ", ".join(missing)
...
def _coerce_numeric_required(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    req = list(_required_lowstate_columns())
    for col in req:
        out[col] = pd.to_numeric(out[col], errors="coerce")
    bad = [c for c in req if not np.isfinite(out[c].to_numpy(dtype=np.float64)).all()]
    if bad:
        raise ValueError(
            "lowstate.csv contains non-finite/non-numeric values in required columns: "
            + ", ".join(bad)
        )
    return out


def discover_lowstate_csv_path(sequence_dir: str | Path) -> Path:
...
def load_prepared_ocelot_lowstate(
    ...
    *,
    ...
    """
    Load and prepare Ocelot lowstate frames for EKF.
    ...
    """
    root = Path(sequence_dir).expanduser().resolve()
    lowstate_path = discover_lowstate_csv_path(root)
    gt_path = _groundtruth_path(root)
    frames_dir = _frames_dir_path(root)

    df = pd.read_csv(lowstate_path)
    _assert_required_columns(df)
    df = _coerce_numeric_required(df)

    # Stable ordering before building dt from sec/nanosec.
    ...
    if verbose:
        print(f"[io] ocelot lowstate: rows={len(df)}, hz~{hz:.2f}, gt={meta['has_groundtruth_csv']}")
